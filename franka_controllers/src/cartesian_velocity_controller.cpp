...
namespace franka_controllers {
    ...
    bool CartesianVelocityController::init(hardware_interface::RobotHW *robot_hardware,
      ...
      if (!node_handle.getParam("/panda_cartesian_velocity_controller/velocity_in_topic", velocity_in_topic)) {
        ROS_ERROR_STREAM(
            "[CartesianVelocityController] YAML config file does not contain parameter 'velocity_in_topic'");
        return false;
      }
    ...
    void CartesianVelocityController::update(const ros::Time & /* time */,
      ...
      for (int i = 0; i < input_velocities.size(); i++) {
        double speed_diff = std::abs(current_velocities_.at(i) - input_velocities.at(i));
        if (speed_diff > acceleration_) {
          if (current_velocities_.at(i) > input_velocities.at(i)) {
            current_velocities_.at(i) =
                current_velocities_.at(i) - (acceleration_ * speed_diff / 0.2); //0.2 - max speed
          } else if (current_velocities_.at(i) < input_velocities.at(i)) {
            current_velocities_.at(i) = current_velocities_.at(i) + (acceleration_ * speed_diff / 0.2);
          } else {
